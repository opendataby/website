...
## 2. Add repositories
echo "deb http://nginx.org/packages/$(echo $(lsb_release -si) | tr '[:upper:]' '[:lower:]')/ $(lsb_release -sc) nginx" | tee /etc/apt/sources.list.d/nginx.list
...
## 6. Configure nginx
#cp /var/www/odata/docroot/odata /etc/nginx/sites-available/default
